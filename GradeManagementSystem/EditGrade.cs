...
namespace GradeManagementSystem
{
    public partial class EditGrade : Form
    {
        ...
        private void editGradeButton_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string updateCourseQuery = "UPDATE grades_Camden440 SET grade = @grade WHERE student_id = @StudentID AND crn = @CRN";
                MySqlCommand cmdUpdateCourse = new MySqlCommand(updateCourseQuery, conn);
                cmdUpdateCourse.Parameters.AddWithValue("@CRN", fixCRN);
                cmdUpdateCourse.Parameters.AddWithValue("@StudentID", fixID);
                cmdUpdateCourse.Parameters.AddWithValue("@grade", newGrade.Text);
            ...
            {
                MessageBox.Show("Error while Editing course: " + ex.Message, "Course Edit Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                conn.Close();
                return;
            }
